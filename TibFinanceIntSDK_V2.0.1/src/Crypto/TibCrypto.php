...
class TibCrypto
{
    ...
    public function performCall($methodName, $data)
    {
        ...
        $session = $this->createSession($this->clientId, $this->userName, $this->password);
        ...
        if (isset($data)) {
            $data_fields = \array_merge($data_fields, $data);
        }

        $content = utf8_encode(json_encode($data_fields));
